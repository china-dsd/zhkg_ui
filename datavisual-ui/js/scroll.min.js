function scrollY(box){	//Y轴滚动
	...
	var scrollY='<div class="scrollboxs" style="width:9px;position:absolute;top:0px;right:0px;z-index:0; border-radius: 5px;background-color:rgba(54, 248, 255, 0.2);height:'+H+'px;">';
		scrollY+='<div id="scrollbtn" style="width:100%;height:'+btnH+'px;margin:0 auto;position:relative;cursor:pointer" tops="0">';
		scrollY+='<div style="background-color:#36f8ff;position:absolute;width:5px;left:2px;height:100%;border-radius:4px"></div>';
		scrollY+='</div></div>';
...
function scrollsY(box){
	...
	$(box).each(function(){
		...
		box.siblings(".scroll_box").css({
			...
			"z-index":0,
			"border-radius": "5px",
			"background-color":"rgba(54, 248, 255, 0.2)",
			"height":H
		});
...
function scrollsX(box){
	...
	$(box).each(function(){
		...
		var scrollX='<div class="scrollX_box" boxW="'+W+'" cententW="'+cententW+'" move="false">';
			...
			scrollX+='<div style="background-color:#36f8ff;position:absolute;width:100%;top:2px;height:5px;border-radius:4px"></div>';
			scrollX+='</div></div>';
		box.parent().css({"position":"relative"});
		if(W/cententW<1){
